...
export const getShoppingCartById = async (req, res) => {
  ...
  try {
    ...
    if (!shoppingCart) {
      ...
      return res
        .status(403)
        .json({ message: "Unauthorized to access this shopping cart" });
    }
  ...
  try {
    ...
    if (existingShoppingCart) {
      ...
      return res.status(409).json({
        message: "An active shopping cart already exists for this user",
      });
    }
...
export const updateShoppingCart = async (req, res) => {
  ...
  try {
    ...
    if (!existingShoppingCart || existingShoppingCart.cart_user_id !== userId) {
      ...
      return res
        .status(403)
        .json({ message: "Unauthorized to update this shopping cart" });
    }
...
export const deleteShoppingCart = async (req, res) => {
  ...
  try {
    ...
    if (!existingShoppingCart || existingShoppingCart.cart_user_id !== userId) {
      ...
      return res
        .status(403)
        .json({ message: "Unauthorized to delete this shopping cart" });
    }
...
export const createCartItem = async (req, res) => {
  ...
  try {
    ...
    const existingCartItem = await isCartItemExistInCart(
      cartId,
      cartit_food_id,
    );
    ...
    if (existingCartItem[0]) {
      return res.status(409).json({
        ...
        cartId,
        cartit_food_id,
        cart_itemid: existingCartItem[1],
      });
    }
...
async function isCartItemExistInCart(cartId, cartit_food_id) {
  try {
    ...
    const existingCartItem = existingShoppingCart.cart_items.find(
      (item) => item.cartit_food_id === cartit_food_id,
    );
    console.log(existingCartItem);
...
export const updateCartItem = async (req, res) => {
  ...
  try {
    ...
    const newCartTotal = shoppingCartWithItems.cart_items.reduce(
      (total, item) => total + item.cartit_total,
      0,
    );
