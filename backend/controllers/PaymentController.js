...
export const createPayment = async (req, res) => {
  ...
  try {
      ...
      shipping_options: [
        {
          shipping_rate_data: {
            type: "fixed_amount",
            fixed_amount: {
              amount: deli_charge * 100,
              currency: "thb",
            },
            display_name: "By delivery service",
            delivery_estimate: {
              minimum: {
                unit: "hour",
                value: 1,
    ...
    return res
      .status(200)
      .json({ id: session.id, confirm: "Payment confirmed" });
...
export const createPaymentOrder = async (
  ...
  try {
    ...
    const orderItems = await Promise.all(
      shoppingCart.cart_items.map(async (cartItem) => {
    ...
    const orderTotal = orderItems.reduce(
      (acc, item) => acc + item.orderit_total,
      0,
    );
