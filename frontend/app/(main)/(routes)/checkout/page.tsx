...
const Checkout = () => {
  ...
  const makePayment = async () => {
    ...
    const stripe = await loadStripe(
      'pk_test_51P1u6ZRtGd9CuWthb5fisAqv5gupcCQQMcHjblEyjI1CtKELC917e5fc9NBYVLNYZa6v1pkIUI2Ijg6kIxpS675W00VjTbkRHw'
    )
  ...
  const mergedAddress = () => {
    if (userData && userData.addressbooks[0]) {
      return (
        userData.addressbooks[0].addb_buildingNo +
        ' ' +
        userData.addressbooks[0].addb_buildingName +
        ' ' +
        userData.addressbooks[0].addb_street +
        ' ' +
        userData.addressbooks[0].addb_prov +
        ' ' +
        userData.addressbooks[0].addb_dist +
        ' ' +
        userData.addressbooks[0].addb_subdist +
        ' ' +
        userData.addressbooks[0].addb_zipcode +
        ' ' +
        '( ' +
        userData.addressbooks[0].addb_directionguide +
        ' Phone:' +
        userData.addressbooks[0].addb_phone +
        ' )'
      )
    }
    return ''
    ...
    <div className="h-full flex flex-col  items-center space-y-4 p-8 md:flex">
            ...
            {Array.isArray(cartData) ? (
              cartData.map(item => (
                <div key={item.cartit_id} className="flex justify-between">
                  <p>
                    {item.quantity} x {item.food.fd_name}
                  </p>
        ...
        <CardFooter>
          <div className="flex justify-between font-medium">
            ...
            <p>
              {': '}{' '}
              {ShoppingCart ? (
                ShoppingCart.cart_total + shippingPrice
              ) : (
                <Spinner />
              )}{' '}
              THB
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <Button
            className="bg-green-600 dark:bg-green-300 hover:bg-secondary-foreground"
            type="submit">
            Place Order
          </Button>
        </form>
